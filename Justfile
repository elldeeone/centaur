...
build:
    ...
    if [[ "${JUST_BUILD_SEQUENTIAL:-0}" =~ ^(1|true|yes)$ ]]; then
      ...
      pids=()
      for recipe in _build-api _build-api-rs _build-iron-proxy _build-slackbot _build-slackbotv2 _build-zulipbot _build-agent; do
        just "$recipe" &
        pids+=("$!")
      done
      status=0
...
_build-zulipbot:
    docker build -t centaur-zulipbot:latest -f services/zulipbot/Dockerfile .
...
_push-registry:
    #!/usr/bin/env bash
    set -euo pipefail
    for img in centaur-api centaur-api-rs centaur-iron-proxy centaur-slackbot centaur-slackbotv2 centaur-zulipbot centaur-agent; do
    ...
    case "{{source}}" in
      ...
      ghcr)
        extra_args+=(
          ...
          --set zulipbot.image.repository=ghcr.io/paradigmxyz/centaur/centaur-zulipbot
